...
for num in range(num_snap):
    ...
    z_scale1 = np.abs(z_sorted[np.int(????*len(z_sorted))])
    z_scale2 = np.abs(z_sorted[np.int(????*len(z_sorted))])
    ...
    z_sorted = zdisk[np.argsort(zdisk)]
    z_scale1 = np.abs(z_sorted[np.int(0.816*len(z_sorted))])
    z_scale2 = np.abs(z_sorted[np.int(0.184*len(z_sorted))])
